...
**Driver slot (scroll sync).** `subscribeToTickerDriver(cb)` registers a callback
that runs **first every frame, unthrottled**, before any subscriber. `ScrollLayout`
drives `lenis.raf(time)` through it so the smooth-scroll engine advances *before*
the scroll-reading subscribers (`ProgressTrigger`) compute their progress — keeping
scroll and the transforms it drives on the **same** rAF tick (ADR-0023). Because
Lenis is a permanent driver, the loop no longer fully idles (it did when Lenis ran
its own rAF too — this is one loop instead of two). Scroll-scrubbed triggers that
must track 1:1 on 120Hz/ProMotion displays should pass `frameInterval={0}`.
