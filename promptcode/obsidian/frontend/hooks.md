...
| `useShowreelLayout` | `use-showreel-layout.ts` | SSR-safe Showreel responsive layout signal — buckets by orientation + width (`mobile`/`tablet`/`desktop`), returns `{ layout, geo }` (the `GEO` set from `utils/showreel/geometry.ts`) and writes the `--sr-*` CSS vars onto the document root. Shared debounced `useSyncExternalStore` store like `useWindowSize`. Call from one consumer (the stage); everything else inherits the vars. See [[home-page]], ADR-0024 |
