...
import { HeroGradient } from "@/showreel/components/3d/hero-gradient";
import { ScrollLetters } from "@/showreel/views/home/scroll-letters";
import {
  heroLetterStyle,
  heroSlidePan,
  ...
  return (
    <div className="relative size-full overflow-hidden rounded-card bg-black">
      ...
      {/* Headline — top-left, sized + tracked to match the marquee. Letters
          ...
          `sr-hero-copy` is the ONE delta from promptcode's hero card: it raises
          the *top* padding floor so the headline clears the A Designer navbar.
          promptcode's header is a floating 42px pill that the headline is meant
          to sit beside; this site's bar is an opaque full-width 73px band, which
          swallowed the first line ("Prompts that") on phones. The rule lives in
          `showreel/showreel.css` and only ever *increases* padding-top — on wide
          viewports it resolves to the original 40px. See that file for the math. */}
      <animated.header
        className="sr-hero-copy pointer-events-none absolute inset-0 z-[3] flex p-[40px] max-sm:p-5"
